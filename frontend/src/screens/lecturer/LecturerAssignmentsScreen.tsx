...
export const LecturerAssignmentsScreen: React.FC = () => {
  ...
  const submitUpload = useCallback(async () => {
    ...
    try {
      setSubmitting(true);
      const fullDescription = `${description?.trim() || ''}${description ? '\n' : ''}Due on: ${dueDate?.toLocaleDateString()}`.trim();
  ...
  }, [courseId, description, dueDate, linkUrl, picked, source, title, token]);
